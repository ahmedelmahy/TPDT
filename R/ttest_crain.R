...
ttest_crain <- function(data, group, timepoints, id, 
  ...
  if(method == "spline") {
    ...
    bas <- create.bspline.basis(range(timepoints), nbasis = length(tau), norder = 4)
    ...
    par1 <- fdPar(bas, 2, lambda = cv_out)
    par2 <- fdPar(bas, 2, lambda = cv_out)
    ...
    for(i in 1:n1) {
      for(j in 1:n2) {
        matsim[l, ] <- eval.fd(finetimes, sm1[i]) - eval.fd(finetimes, sm2[j])
        l <- l + 1
      }
    }  
...
if(method == "rawdata") {
  ...
  bas <- create.bspline.basis(range(timepoints), nbasis = length(timepoints), norder = 4)
  ...
  mclapply(1:nboot, function(b) {
    ...
    # step1
    dn <- rmvnorm(1, dbar, Sig, method = "svd")
    # step2
...
#        main = paste0(unique(group)[1], " vs. ", unique(group)[2], 
